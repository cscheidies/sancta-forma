...
def step(state: State, direction: str) -> Optional[State]:
    ...
    if state.costumed:
        if target != state.element:
            return None
        # Cleanse during costume: 0 pts, reset corruption to 0
        return State(pos=(nr, nc), element=state.element, corruption=0,
                     score=state.score, grid=new_grid, costumed=False)
    ...
    if target in HUNTERS:
        ...
        return State(pos=(nr, nc), element=state.element,
                     corruption=state.corruption,
                     score=state.score + score_delta,
                     grid=new_grid, costumed=True)

    # SACRED SHAPE ABSORPTION
    score_delta, corr_op = ABSORB[(state.element, target)]
    new_corr = 0 if corr_op == "cleanse" else state.corruption + corr_op

    if new_corr >= DEATH[state.element]:
        return None  # corruption death

    return State(pos=(nr, nc), element=state.element, corruption=new_corr,
                 score=state.score + score_delta, grid=new_grid, costumed=False)
...
def has_valid_move(state: State) -> bool:
    return any(step(state, d[0]) is not None for d in DIRS)
...
def solve(level, max_depth=30, max_states=200000):
    ...
    if is_win(start):
        return {"solvable": True, "shortest_path": [], "depth": 0,
                "states_explored": 1, "corruption_required": False, "hunter_required": False}
    ...
    for end_state, path in winning_paths[:500]:
        took_corruption = took_hunter = False
        s = start
        for d in path:
